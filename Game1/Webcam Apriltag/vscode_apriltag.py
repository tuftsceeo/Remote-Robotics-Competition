...
class PerspectiveSelector:
    def __init__(self, frame_shape):
        ...
        self.output_size = (800, 600)  # Default output size
...
class AprilTagDetector:
    def __init__(self, channel_client):
        ...
        self.cap = cv2.VideoCapture(1) # change to 1 for external webcam
    ...
    def toggle_crop_mode(self):
        ...
        if self.crop_mode_enabled:
            ...
            self.scale_x = 1920 / self.perspective_selector.output_size[0]
            self.scale_y = 1080 / self.perspective_selector.output_size[1]
        else:
    ...
    def draw_detections(self, frame, tags):
        ...
        for tag in tags:
            ...
            cv2.putText(frame, rotation_text,
        ...
        try:
            while not stop_event.is_set():
                ...
                if self.crop_mode_enabled and self.perspective_selector.transform_matrix is not None:
                    frame = self.perspective_selector.apply_transform(frame)
                
                # Detect AprilTags (on cropped frame if crop mode is enabled)
                ...
                # Draw detections on the appropriate frame
                if self.crop_mode_enabled and self.perspective_selector.transform_matrix is not None:
                    frame = self.draw_detections(frame, tags)
                    display_frame = frame  # Use cropped frame for display
                else:
                    display_frame = self.draw_detections(display_frame, tags)
                
                # Draw selection overlay on original frame if not in crop mode
                ...
                cv2.putText(display_frame, f"Tags: {len(tags)}", 
                           (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                cv2.putText(display_frame, f"Topic: /{self.selected_topic}/All", 
                           (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                
                # Add crop mode status
                mode_text = "CROP MODE" if self.crop_mode_enabled else "FULL VIEW"
                mode_color = (0, 255, 0) if self.crop_mode_enabled else (255, 255, 255)
                ...
                if self.perspective_selector.selecting:
                    cv2.putText(display_frame, "SELECTING AREA - Click 4 corners", 
                               (10, display_frame.shape[0] - 50), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
                elif self.perspective_selector.transform_matrix is not None:
                    cv2.putText(display_frame, "Area selected - Press 'c' to toggle crop", 
                               (10, display_frame.shape[0] - 50), 
...
class DetectorGUI:
    def __init__(self):
        ...
        self.detector_thread = Thread(target=self.detector.run, args=(self.stop_event,))
    ...
    def run(self):
        ...
        def update_gui():
            self.update_status()
            self.root.after(1000, update_gui)  # Update every second
